...
async function setBuildingImages() {
  try {
    ...
    // Update HSR Layout
    const hsrResult = await buildingsCollection.updateOne(
      { name: 'Hodo Stays HSR Layout' },
      { $set: { images: hsrImages } }
    );
    console.log(`✅ Updated HSR Layout: ${hsrResult.modifiedCount} document(s) modified`);

    // Update JP Nagar
    const jpResult = await buildingsCollection.updateOne(
      { name: 'Hodo Stays JP Nagar' },
      { $set: { images: jpImages } }
    );
    console.log(`✅ Updated JP Nagar: ${jpResult.modifiedCount} document(s) modified`);
